    ...
    private static boolean loadFromZip(Context context, String libname, File destLocalFile, String arch) {
        ...
        try {
            ...
            if (entry == null) {
                ...
                if (entry == null) {
                    entry = zipFile.getEntry("libs/" + arch + "/" + libname + ".so");
                    if (entry == null) {
                        entry = zipFile.getEntry("libs/" + arch + "/" + libname);
                        if (entry == null) {
                            throw new Exception("CustomNativeLoader: Unable to find file in apk:" + "lib/" + arch + "/" + libname);
                        }
                    }
                }
            }
